<?php if (!defined('THINK_PATH')) exit();?>		
		<!--头部和尾部-->
		<link rel="stylesheet" type="text/css" href="/ruopusuye/Public/css/public.css"/>
		<!--首页页面-->
		<link rel="stylesheet" type="text/css" href="/ruopusuye/Public/css/index.css"/>
		<!--首页-->
		<script type="text/javascript" src="/ruopusuye/Public/js/public.js"></script>
		<script type="text/javascript" src="/ruopusuye/Public/js/lunbo.js"></script>
		<script src="/ruopusuye/Public/js/jquery-1.8.3/jquery.min.js"></script>
<script type="text/javascript">
    window.onload = function(){
        showen();
    };
	$(document).ready(function () {
            scroll();
			top_1();
			lunbo();
    });
</script>
	</head>
	<body>
	
<!--头部开始-->
		<div id="header">
			<div class="header w">
				<div class="header_logo floatL"><img src="/ruopusuye/Public/img/logo.jpg"/></div>
				<div class="header_iphone floatL">
					<p>24小时服务热线</p>
					<p class="green bg">053-6521-7508</p>
				</div>
				<div class="header_choose floatL">
					<a class="black" href="#">中文</a>
					<span>/</span>
					<a href="#">英文</a>
				</div>
			</div>
		</div>
<!--导航栏开始-->
		<div id="nav" class="bg">
			<ul class="w">
				<li><a href="index.html">首页</a></li>
				<li><a href="introduction.html">公司简介</a></li>
				<li><a href="news.html">新闻中心</a></li>
				<li><a href="product.html">产品展示</a></li>
				<li><a href="contact.html">联系我们</a></li>
				<li class="addWidth">
					<div class="li_input">
						<div class="li_text"><input type="text" name="" placeholder="请输入关键词" value="" /></div>
						<div class="li_button"><input type="button" name="" value="搜索"/></div>
					</div>
				</li>
			</ul>
		</div>
<!--导航栏结束-->
<!--轮播图开始-->
		<div id="lunbobox">
		        <div id="toleft">&lt;</div>
		        <div class="lunbo">
		                <a href="#"><img src="/ruopusuye/Public/img/index_lunbo_1.jpg"></a>
		                <a href="#"><img src="/ruopusuye/Public/img/index_lunbo_2.jpg"></a>
		                <a href="#"><img src="/ruopusuye/Public/img/index_lunbo_3.jpg"></a>
		        </div>
		        <div id="toright">&gt;</div>
		        <ul>
		            <li></li>
		            <li></li>
		            <li></li>
		        </ul>
		        <span></span>
    </div>
<!--轮播图结束-->

<!--内容开始-->
		<div id="main" class="w">
			<ul class="w">
			<!--公司简介-->
				<li class="main_1 w">
					<div class="main_1_top">
						<img src="/ruopusuye/Public/img/index_main_1.png"/>
						<p>公司简介</p>
					</div>
					<div class="main_1_bottom w">
						<div class="main_1_bottom_LBox">
							<img src="img/index_main_简介.jpg"/>
						</div>
						<div class="main_1_bottom_RBox">
							<h1>寿光市若普有限责任公司</h1>
							<p>寿光市若普塑业有限责任公司坐落于风景秀美的渤海之滨，有着“菜都”之美誉“，中国冬暖式蔬菜大棚发源地”的潍坊寿光。公司自成立至今，引进国外先进技术，是国内首家从事PET材料绳索的中外合资企业，经过不断创新，已拥有较为优质的技术力量与好的销售、技术团队。经过不断的创新和发展，各方面取得了很多国内外客户的认可，我们也将继续秉持用心制造更高品质的产品与服务经营理念，以迎取客户更大的满意与信赖。</p>
							<a class="more" href="introduction.html">查看更多</a>
						</div>
					</div>
				</li>
			<!--产品展示-->
				<li class="main_2 w">
					<div class="main_2_top">
						<img class="box" src="/ruopusuye/Public/img/index_main_2.png"/>
						<p>产品展示</p>
					</div>
					<div class="main_2_bottom w">
						<ul>
						    <li class="liss">
						    	<img src="/ruopusuye/Public/img/index_main_pre1.jpg" alt="">
						    	<h4>PET压膜绳</h4>
						    	<a href="product.html">查看详情</a>
						    </li>
						   	<li class="liss">
						    	<img src="/ruopusuye/Public/img/index_main_pre2.jpg" alt="">
						    	<h4>PET船用渔网线</h4>
						    	<a href="product.html">查看详情</a>
						    	
						    	
						    </li>
						    <li class="liss">
						    	<img src="/ruopusuye/Public/img/index_main_pre3.jpg" alt="">
						    	<h4>盐场专用绳</h4>
						    	<a href="product.html">查看详情</a>
						    	
						    </li>
						    <li class="liss">
						    	<img src="/ruopusuye/Public/img/index_main_pre4.jpg" alt="">
						    	<h4>海上养殖专用用绳</h4>
						    	<a href="product.html">查看详情</a>
						    	
						    </li>
						  
						</ul>
						
					</div>
				</li>
			<!--新闻资讯-->
				<li class="main_4 w">
					<div class="main_2_top w">
						<img src="/ruopusuye/Public/img/index_main_3.png"/>
						<p>新闻资讯</p>
					</div>
					<div class="main_4_center w">
						<a class="main_4_center_Box w" href="newspage.html">
							<img src="img/1.png"/>
							<h6>寿光市若普塑业有限责任公司</h6>
							<p>寿光市若普塑业有限责任公司坐落于风景秀美的渤海之滨，有着“菜都”之美誉“，中国冬暖式蔬菜大棚发源地”的潍坊寿光。公司自成立至今，引进国外先进技术，是国内首家从事PET材料绳索的中外合资企业，经过不断创新，已拥有较为优质的技术力量与好的销售、技术团队。经过不断的创新和发展，各方面取得了很多国内外客户的认可，我们也将继续秉持用心制造更高品质的产品与服务经营理念，以迎取客户更大的满意与信赖。</p>
						</a>
						<a class="main_4_center_Box w" href="newspage.html">
							<img src="img/2.png"/>
							<h6>寿光市若普塑业有限责任公司</h6>
							<p>寿光市若普塑业有限责任公司坐落于风景秀美的渤海之滨，有着“菜都”之美誉“，中国冬暖式蔬菜大棚发源地”的潍坊寿光。公司自成立至今，引进国外先进技术，是国内首家从事PET材料绳索的中外合资企业，经过不断创新，已拥有较为优质的技术力量与好的销售、技术团队。经过不断的创新和发展，各方面取得了很多国内外客户的认可，我们也将继续秉持用心制造更高品质的产品与服务经营理念，以迎取客户更大的满意与信赖。</p>
						</a>
						<a class="main_4_center_Box w" href="newspage.html">
							<img src="img/3.png"/>
							<h6>寿光市若普塑业有限责任公司</h6>
							<p>寿光市若普塑业有限责任公司坐落于风景秀美的渤海之滨，有着“菜都”之美誉“，中国冬暖式蔬菜大棚发源地”的潍坊寿光。公司自成立至今，引进国外先进技术，是国内首家从事PET材料绳索的中外合资企业，经过不断创新，已拥有较为优质的技术力量与好的销售、技术团队。经过不断的创新和发展，各方面取得了很多国内外客户的认可，我们也将继续秉持用心制造更高品质的产品与服务经营理念，以迎取客户更大的满意与信赖。</p>
						</a>

						
					</div>
					<div class="main_4_bottom">
						<a class="more" href="news.html">更多新闻</a>
					</div>

					
				</li>
			<!--企业优势-->
				<li class="main_3 w">
					<div class="main_2_top">
						<img src="img/index_main_4.png"/>
						<p>企业优势</p>
					</div>
					<div class="main_3_bottom w">
						<div class="main_3_bottom_Box">
							<img src="img/优势_1.jpg"/>
							<p>图片：（350-380）*（200-300）</p>
						</div>
						<div class="main_3_bottom_Box">
							<img src="img/优势_3.jpg"/>
							
							<p>图片：（350-380）*（200-300）</p>
						</div>
						<div class="main_3_bottom_Box">
							<img src="img/优势_2.jpg"/>
							<p>图片：（350-380）*（200-300）</p>
						</div>
						
					</div>
				</li>
			
			</ul>
		</div>
<!--内容结束-->
<!--尾部开始-->
		<div id="footer">
			<div class="footer w">
				<div class="footer_LBox">
						<!--<p>寿光市若普塑业有限责任公司</p>-->
					<img src="img/logo_1.png"/>
					<!--<p>主要产品：以聚酯切片（PET）为主要原料的PET压膜绳、线，PET船用渔网绳，海上养殖专用用绳，盐场专用绳等一系列行业用绳。</p>-->
					<!--<p>产品特性：此类用绳均采用先进工艺，使得产品具有强度高，不伸缩，防酸碱，耐高温，抗紫外线，施工简便，应用成本低，抗老化、防老化，使用寿命长（自然环境寿命使用15年以上）。</p>-->
					<!--<p>可根据客户需求订制各种不同规格及不同型号专用绳，以此满足客户不同需求。</p>-->
				</div>
				<div class="footer_RBox">
					<ul id="uls">
						<li>
							<div class="footer_RBox_img"><img src="img/index_footer_qq.png"/></div>
							<div class="footer_RBox_span"><span>联系QQ：403539296</span></div>
						</li>
						<li>
							
							<div class="footer_RBox_img"><img src="img/index_footer_iPhone.png"/></div>
							<div class="footer_RBox_span"><span>联系电话：05365217508&nbsp;&nbsp;&nbsp;&nbsp;15263647456</span></div>
						</li>
						<li>
							
							<div class="footer_RBox_img"><img src="img/index_footer_email.png"/></div>
							<div class="footer_RBox_span"><span>联系邮箱：403539296@qq.com</span></div>
						</li>	
						<li>
							
							<div class="footer_RBox_img"><img src="img/index_footer_address.png"/></div>
							<div class="footer_RBox_span"><span>联系地址：山东省潍坊市寿光市孙家集街道黄埔村118号</span></div>
						</li>
						<!--<li>4</li>-->
					</ul>
				</div>
			</div>
		</div>
		<div id="footer_1" class="bg">
			<div class="footer_1 w">
				@版权所有：寿光市若普塑业有限责任公司
				<a href="http://www.miitbeian.gov.cn/">鲁ICP备06041183号-2</a>
				技术支持：千行软件工作室
			</div>
		</div>
<!--尾部结束-->
<!--小插件-->
<a id="top_top" href="#">
	<img src="img/index_fixed_top.png" alt="" />
</a>
<a id="top_weixin" href="#">
	<img src="img/index_fixed_weixin.png" alt="" />
</a>
<a id="top_iphone" target=blank href=tencent://message/?uin=2234461543&Site=im.qq.com&Menu=yes href="#">
	<img src="img/index_fixed_iphone.png" alt="" />
</a>
<div id="kongbai_weixin"></div>
<div id="kongbai_iphone">
	<h5>电话热线：</h5>
	<p>053-6521-7508</p>
	<h5>手机号：</h5>
	<p>15263647456</p>
</div>
<div id="kefu">
	<h2 class="tact"><img src="img/QQzxkf.jpg"><span id="cancel">×</span></h2>
	<a class="kefu_0"  target="blank" href="tencent://message/?uin=2234461543&Site=im.qq.com&Menu=yes"><img src="img/kefu_11.jpg"></a>
	<h2 class="times">工作时间</h2>
	<div id="gzsj">7*24小时全天在线</div>
</div>

<!--小插件结束-->
	</body>
</html>