            ...
            <?php } ?>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<input type="text" name="name" value="<?php echo ($data[title]); ?>" autocomplete="off" placeholder="请输入新闻标题" class="layui-input" lay-verify="fname">
					</div>
				</div>
					<div class="layui-inline">
						<label class="layui-form-label">发布时间</label>
						<div class="layui-input-block">
							<input type="text" name="time" id="date" value="<?php echo (date("Y-m-d",$data[time] )); ?>" placeholder="请选择发布时间" autocomplete="off" class="layui-input sj" onclick="layui.laydate({elem: this})">
						</div>
					</div>
				<div class="layui-form-item">
					<label class="layui-form-label">首页展示</label>
					<div class="layui-input-block" id="indexShow">
					<?php if($data["state"] == '1'): ?><input type="radio" name="state" value="1"  title="是" lay-filter="yesno" checked="checked">
						<input type="radio" name="state" value="2"  title="否" lay-filter="yesno">
					<?php else: ?>
						<input type="radio" name="state" value="1"  title="是" lay-filter="yesno">
						<input type="radio" name="state" value="2"  title="否" lay-filter="yesno" checked="checked" ><?php endif; ?>
				</div>
				</div>
				<div class="layui-form-item" id="tututu" >
					...
					<div style="text-align: center;">
						<img src="/ruopusuye/Public/Uploads/<?php echo ($data["picture"]); ?>" alt="" style="width: 300px;">
					</div>
				</div>
				<div class="layui-form-item">
					<label class="layui-form-label">新闻分类</label>
					<div class="layui-input-block">
					<?php if($data["classification"] == '1'): ?><input type="radio" name="classification" value="1" title="行业新闻" checked="checked">
						<input type="radio" name="classification" value="2" title="公司动态">
					<?php else: ?>
						<input type="radio" name="classification" value="1" title="行业新闻">
						<input type="radio" name="classification" value="2" title="公司动态" checked="checked"><?php endif; ?>
				</div>
				</div>
				  <div class="layui-form-item layui-form-text">
    <label class="layui-form-label">内容简介</label>
    <div class="layui-input-block">
      <textarea placeholder="请输入内容" name="abstracts" class="layui-textarea"><?php echo ($data["abstracts"]); ?></textarea>
    </div>
  </div>
				<div class="layui-form-item">
